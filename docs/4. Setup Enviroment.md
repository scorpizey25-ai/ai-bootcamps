...
### 🔹 Backup Environment
Jika ingin memindahkan environment ke laptop lain, backup dulu:
```bash
conda activate ai_env
conda env export > ai_env.yml
```
👉 File `ai_env.yml` berisi semua library + versinya.  

### 🔹 Restore di Laptop Baru
1. Install Miniconda / Anaconda di laptop baru.
2. Pindahkan file `ai_env.yml` ke laptop baru.
3. Jalankan:
   ```bash
...
Jika hanya mau daftar package via `pip`:
```bash
...
⚠️ **Tips:** Backup dengan `.yml` lebih disarankan karena lebih konsisten menjaga versi library.
...
- Environment bisa di-backup & restore ke laptop lain dengan mudah.
